...
namespace sample1
{
    class Program
    {
        ...
        {
            Tuple<double, double>[] patients = new Tuple<double, double>[size + Program.edgePatients.Length];
            ...
            {
                patients[i] = Program.edgePatients[Math.Abs(size - i)];
                i++;
            }
        ...
        static double calculate_BMI(double height, double weight)
        {
            ...
            {
                Console.WriteLine("Height: " + height + " Weight: " + weight + " BMI:invalid " );
                return 0;
            }
